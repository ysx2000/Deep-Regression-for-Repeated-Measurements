...
def mean_fun1(x):
    """
    Calculate the true mean function in Case S.2. 
    $$f(x) =  (x_1^2x_2^3 + \log(1+x_3) + \sqrt{1+x_4x_5} +\exp(x_5/2))^2 - 12$$
    ...
    Returns: f(x)
    """
    return ( (x[0]*x[0]*x[1]*x[1]*x[1] + math.log(1+x[2]) + math.sqrt(1+x[3]*x[4]) + math.exp(x[4]/2)) ** 2 - 12)
...
def savedata(seed):
    ...
    np.save("./Simulation/CaseS2/data/data"+str(seed)+".npy", a)
