...
def mean_onedim_fun1(x, a = 0.3, b = 1.6):
    ksize = 20
    i1 = np.arange(ksize)
    reswei = np.sum( np.power((a), i1) * np.cos(np.multiply(np.power(b,i1),2*math.pi* x[0])) + np.power((a), i1) * np.cos(np.multiply(np.power(b,i1), 2*math.pi*x[1])) + np.power((a), i1) * np.cos(np.multiply(np.power(b,i1), 2*math.pi*x[2])) )
    # print("h")
    return reswei
...
def savedata(seed):
    ...
    np.save("./Simulation/Case4/data/data"+str(seed)+".npy", a)
