...
def mean_onedim_fun1(x, a = 0.3, b = 1.6):
    ...
    reswei = np.sum( np.power((a), im) * np.cos(np.multiply(np.power(b,i1),2*math.pi* x[0])) * np.cos(np.multiply(np.power(b,i2), 2*math.pi*x[1])) * np.cos(np.multiply(np.power(b,i3), 2*math.pi*x[2])) )
    return reswei 
...
def generate_onedim_fun1(n,m): 
    ...
    for i in range(n):
        ...
        for k in range(20): 
            yi = yi + (a[k] * np.sin((k+1) * math.pi * x[i,:,0]) + b[k] * np.sin((k+1) * math.pi * x[i,:,1]) + c[k] * np.sin((k+1) * math.pi * x[i,:,2]) + a2[k] * np.cos((k+1) * math.pi * x[i,:,0]) + b2[k] * np.cos((k+1) * math.pi * x[i,:,1]) + c2[k] * np.cos((k+1) * math.pi * x[i,:,2]) ) /(k+1)   #np.cos(x[i]) + c * 2*x[i]+ d * np.cos(2*x[i]) 
        y = np.append(y,yi) 
        print(i) 
    y = y.reshape((n,m)) 
...
def savedata(seed):
    ...
    x_test1 = np.random.uniform(low=0, high=1, size=10000)
    x_test = np.empty((len(x_test1), 3))
    x_test[:,0] = np.cos(x_test1*2*math.pi)
    x_test[:,1] = np.sin(x_test1*2*math.pi)
    x_test[:,2] = x_test1
    ...
    np.save("./Simulation/Case6/data/data"+str(seed)+".npy", a)
